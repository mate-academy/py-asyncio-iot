...
class HueLightDevice:
    ...
    async def send_message(self, message_type: MessageType, data: str = "") -> None:
        ...
        await asyncio.sleep(TIME_TO_SLEEP)
        print(f"Hue Light received message of type {message_type.name}.")
...
class SmartSpeakerDevice:
    ...
    async def send_message(self, message_type: MessageType, data: str = "") -> None:
        ...
        await asyncio.sleep(TIME_TO_SLEEP)
        print(f"Smart Speaker received message of type {message_type.name}.")
...
class SmartToiletDevice:
    ...
    async def send_message(self, message_type: MessageType, data: str = "") -> None:
        ...
        if message_type == MessageType.FLUSH:
            await asyncio.sleep(FLUSH_TIME_TO_SLEEP)
        else:
            await asyncio.sleep(TIME_TO_SLEEP)
        print(f"Smart Toilet received message of type {message_type.name}.")
