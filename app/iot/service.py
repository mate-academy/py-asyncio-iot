...
class IOTService:
    ...
    async def register_device(self, devices: list[Device]) -> list[str]:
        async def register_single_device(device: Device) -> str:
            await device.connect()
            device_id = await generate_id()
            self.devices[device_id] = device
            return device_id

        coroutines = [register_single_device(device) for device in devices]
        device_ids = await asyncio.gather(*coroutines)
        return device_ids
